...
const templateWithJQuery = () => {
  ...
  for (let i = 0; i < starWarsPeople.length; i++) {
    const firstName = starWarsPeople[0];
    const secondHeight=starWarsPeople[1];
    const thirdColor=starWarsPeople[2];

    let getTemplet = $('#template').clone();
    getTemplet.find('h2').text(firstName.name);
    getTemplet.find('h3').text(secondHeight.height);
    getTemplet.find('p').text(thirdColor.eye_color);
    $('main').append(getTemplet);
...
const howMuchPencil = (str) => {
  ...
  // Solution code here...
   for (let i = 0; i <=str.length; i++) {
     const element = str.slice(i,str.length);
     result.push(element);
   }
  return result;
};
...
const listFoods = (recipe) => {
  ...
  recipe.ingredients.forEach(element => {
    let amountOfUnit = element.slice(element.indexOf(' ')+1);
    result.push(amountOfUnit.slice(amountOfUnit.indexOf(' ') + 1));
  });
  return result;
};
